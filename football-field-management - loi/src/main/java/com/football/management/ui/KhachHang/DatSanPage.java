...
public class DatSanPage {
    ...
    public static Node createView(SanKhachHangRow sanMacDinh,
        ...
        ngayMacDinh != null ? ngayMacDinh : LocalDate.now().plusDays(1)
);
...
if (gioBDMacDinh != null && !gioBDMacDinh.isBlank()) {
    txtGioBD.setText(gioBDMacDinh);
}
...
if (gioKTMacDinh != null && !gioKTMacDinh.isBlank()) {
    txtGioKT.setText(gioKTMacDinh);
}
    ...
    /**
 * Mở trang đặt sân với sân mặc định nhưng chưa có ngày/giờ mặc định
 */
public static Node createView(SanKhachHangRow sanMacDinh) {
    return createView(sanMacDinh, null, null, null);
}

/**
 * Mở trang đặt sân trực tiếp từ menu
 */
public static Node createView() {
    return createView(null, null, null, null);
}
}