...
public class KhachHangDatSanDAO {
    ...
    /** Lấy danh sách sân còn trống theo ngày và khung giờ khách hàng chọn */
public List<SanKhachHangRow> timSanTrongChoKhachHang(LocalDate ngayDat,
                                                     String gioBatDau,
                                                     String gioKetThuc) throws SQLException {
    List<SanKhachHangRow> ds = new ArrayList<>();
    ...
    String sql = """
            SELECT sb.ma_san,
                   ...
                   (
                       ...
                       WHERE bg.ma_san = sb.ma_san
                         AND bg.dang_ap_dung = 1
                         AND bg.ngay_ap_dung_tu <= ?
                         AND (bg.ngay_ap_dung_den IS NULL OR bg.ngay_ap_dung_den >= ?)
                         AND (
                             bg.thu_trong_tuan IS NULL
                             OR bg.thu_trong_tuan = TO_NUMBER(TO_CHAR(?, 'D'))
                         )
                         AND TO_DATE(bg.gio_bat_dau, 'HH24:MI') <= TO_DATE(?, 'HH24:MI')
                         AND TO_DATE(bg.gio_ket_thuc, 'HH24:MI') >= TO_DATE(?, 'HH24:MI')
                   ) AS gia_hien_tai
            FROM san_bong sb
            JOIN loai_san ls ON sb.ma_loai_san = ls.ma_loai_san
            WHERE sb.trang_thai_san NOT IN ('NGUNG_HOAT_DONG', 'BAO_TRI')

              -- Sân phải mở cửa trước hoặc đúng giờ khách muốn bắt đầu
              AND (
                  sb.gio_mo_cua IS NULL
                  OR TO_DATE(sb.gio_mo_cua, 'HH24:MI') <= TO_DATE(?, 'HH24:MI')
              )

              -- Sân phải đóng cửa sau hoặc đúng giờ khách muốn kết thúc
              AND (
                  sb.gio_dong_cua IS NULL
                  OR TO_DATE(sb.gio_dong_cua, 'HH24:MI') >= TO_DATE(?, 'HH24:MI')
              )

              -- Không được có đơn đặt sân khác bị trùng giờ
              AND NOT EXISTS (
                  SELECT 1
                  FROM dat_san ds
                  WHERE ds.ma_san = sb.ma_san
                    AND ds.ngay_dat = ?
                    AND ds.trang_thai_dat_san NOT IN ('DA_HUY')
                    AND TO_DATE(?, 'HH24:MI') < TO_DATE(ds.gio_ket_thuc, 'HH24:MI')
                    AND TO_DATE(?, 'HH24:MI') > TO_DATE(ds.gio_bat_dau, 'HH24:MI')
              )
            ORDER BY sb.ma_san
            """;
    ...
    try (Connection conn = DBConnection.getConnection();
        ...
        String gioBD = gioBatDau.trim();
        String gioKT = gioKetThuc.trim();

        // Tham số cho phần lấy giá theo ngày + giờ
        ps.setDate(1, sqlNgay);
        ps.setDate(2, sqlNgay);
        ps.setDate(3, sqlNgay);
        ps.setString(4, gioBD);
        ps.setString(5, gioKT);

        // Tham số kiểm tra giờ mở cửa / đóng cửa của sân
        ps.setString(6, gioBD);
        ps.setString(7, gioKT);

        // Tham số kiểm tra trùng lịch trong bảng dat_san
        ps.setDate(8, sqlNgay);
        ps.setString(9, gioBD);
        ps.setString(10, gioKT);
